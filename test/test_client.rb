...
class TestIntegration < MiniTest::Unit::TestCase
  ...
  def setup
    ...
    stub_request(:post, "http://import.brassring.com/WebRouter/WebRouter.asmx/route").
      with(:body => fixture('page_2_request.xml')).
      to_return(:status => 200, :body => fixture('search_results_page_2.xml'), :headers => {})

    stub_request(:get, "https://sjobs.brassring.com/1033/ASP/TG/cim_jobdetail.asp?jobid=216081&partnerid=25152&siteid=5244").
      with(:headers => {'Accept'=>'*/*'}).
      to_return(:status => 200, :body => fixture('job_details_page_2.html'), :headers => {})
  ...
  def test_jobs_with_page
    client = Kenexa::Client.new
    jobs = client.jobs(:page => 2)
    job = jobs.first

    assert_equal 50, jobs.size
    assert_equal 'Senior Maintenance Manager II', job.title

    jobs = client.jobs(:page => 3)
    job = jobs.first
